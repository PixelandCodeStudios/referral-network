...
async function qrScanFlow(browser) {
  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      const websiteLink = links.find(a => a.textContent.includes('Visit Website') || a.href.includes('myhst.com'));
...
async function contactSubmitFlow(browser) {
  ...
  try {
    ...
    await page.type('textarea[name="message"]', 'This is a test message from stress testing.', { delay: 50 });
    await sleep(1000);
