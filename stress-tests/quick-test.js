...
async function sendTestEvent() {
  const event = {
    ...
    partner_id: ["brian-dow", "joshua-naylor", "tiffany-mcalister", "tom-berry"][
...
async function runTest() {
  ...
  console.log(
    `Successful:        ${chalk.green(stats.successful)} (${((stats.successful / stats.total) * 100).toFixed(1)}%)`
  );
  console.log(
    `Failed:            ${stats.failed > 0 ? chalk.red(stats.failed) : chalk.green(stats.failed)}`
  );
  console.log("");
