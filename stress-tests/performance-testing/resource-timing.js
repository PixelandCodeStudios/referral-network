...
async function testPage(browser, pageInfo) {
  ...
  try {
    ...
    console.log(`    FCP:  ${metrics.fcp ? formatMetric(metrics.fcp, thresholds.pageLoad.fcp) + 'ms' : 'N/A'}`);
...
function generateSummary(results) {
  ...
  const avgFCP = validResults.reduce((sum, r) => sum + (r.metrics.fcp || 0), 0) / validResults.length;
  const avgLCP = validResults.reduce((sum, r) => sum + r.metrics.lcp, 0) / validResults.length;
  const avgSize = validResults.reduce((sum, r) => sum + r.metrics.totalTransferSize, 0) / validResults.length;

  console.log(chalk.bold('Average Metrics:'));
  console.log(`  TTFB: ${formatMetric(avgTTFB, thresholds.pageLoad.ttfb)}ms (target: <${thresholds.pageLoad.ttfb}ms)`);
  console.log(`  FCP:  ${formatMetric(avgFCP, thresholds.pageLoad.fcp)}ms (target: <${thresholds.pageLoad.fcp}ms)`);
  console.log(`  LCP:  ${formatMetric(avgLCP, thresholds.pageLoad.lcp)}ms (target: <${thresholds.pageLoad.lcp}ms)`);
