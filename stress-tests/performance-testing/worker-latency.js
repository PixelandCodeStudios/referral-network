...
function compareResults(nonNotifiable, notifiable, contacts) {
  ...
  console.log(
    `   Mean difference: ${chalk.yellow("+" + meanDiff.toFixed(2) + "ms")} (${chalk.yellow("+" + meanOverhead.toFixed(1) + "%")})`
  );
  console.log(
    `   p95 difference:  ${chalk.yellow("+" + p95Diff.toFixed(2) + "ms")} (${chalk.yellow("+" + p95Overhead.toFixed(1) + "%")})`
  );
  ...
  if (meanOverhead > 200) {
    ...
    console.log(
      chalk.red("   Synchronous email sending is significantly impacting response times.")
    );
    console.log(
      chalk.yellow(
        "   RECOMMENDATION: Move email notifications to async queue (Cloudflare Queues).\n"
      )
    );
    ...
    console.log(
      chalk.yellow("   RECOMMENDATION: Consider async processing for better performance.\n")
    );
  } else {
  ...
  console.log(
    `   Improvement:    ${chalk.green("-" + estimatedImprovement.toFixed(2) + "ms")} (${chalk.green("-" + p95Overhead.toFixed(1) + "%")})\n`
  );
...
async function main() {
  ...
  console.log(
    chalk.gray("This test measures the impact of email sending on worker response times.\n")
  );
