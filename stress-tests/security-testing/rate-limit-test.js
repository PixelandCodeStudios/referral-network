...
async function simulateBurstAttack() {
  ...
  console.log(`  Accepted: ${chalk.yellow(accepted)} (${((accepted/burstSize)*100).toFixed(1)}%)`);
  console.log(`  Rejected: ${chalk.green(rejected)} (${((rejected/burstSize)*100).toFixed(1)}%)`);
...
async function calculateQuotaExhaustionTime() {
  ...
  console.log(`  Time to exhaust quota: ${chalk.red(timeToExhaustQuota.toFixed(1) + ' seconds')} (${chalk.red((timeToExhaustQuota/60).toFixed(1) + ' minutes')})`);

  if (timeToExhaustQuota < 1800) { // Less than 30 minutes
...
async function testRecommendedRateLimits() {
  ...
  console.log(`  Per IP: ${recommended.perIP.requests} requests per ${recommended.perIP.window} seconds`);
  console.log(`  Per Session: ${recommended.perSession.requests} requests per ${recommended.perSession.window} seconds\n`);
  ...
  console.log(`  Daily usage: ${chalk.green((requestsSent / (elapsed / 86400)).toFixed(0) + ' requests/day')}`);
  console.log(`  D1 writes used: ${chalk.green((requestsSent / D1_DAILY_WRITE_LIMIT * 100).toFixed(1) + '%')} of daily quota\n`);

  const daysToExhaustQuota = D1_DAILY_WRITE_LIMIT / (requestsSent / (elapsed / 86400));

  console.log(chalk.bold('Protection Analysis:\n'));
  console.log(`  Time to exhaust quota (single attacker): ${chalk.green(daysToExhaustQuota.toFixed(1) + ' days')}`);
...
function generateSecurityReport(burstResults, quotaResults, limitsResults) {
  ...
  console.log(`  Quota exhaustion time: ${chalk.red(quotaResults.timeToExhaustQuota.toFixed(1) + ' seconds')} (${chalk.red((quotaResults.timeToExhaustQuota/60).toFixed(1) + ' minutes')})`);
  console.log(`  Attack rate: ${chalk.red(quotaResults.requestsPerSecond.toFixed(2) + ' req/sec')}`);
  ...
  console.log(`  Quota exhaustion time: ${chalk.green(limitsResults.daysToExhaustQuota.toFixed(1) + ' days')}`);
  ...
  const improvement = (quotaResults.timeToExhaustQuota / 60) / (limitsResults.daysToExhaustQuota * 24 * 60);
  console.log(`  Protection multiplier: ${chalk.green((limitsResults.daysToExhaustQuota * 24 * 60 / (quotaResults.timeToExhaustQuota / 60)).toFixed(0) + 'x')}`);
  console.log(`  From ${chalk.red((quotaResults.timeToExhaustQuota/60).toFixed(0) + ' minutes')} to ${chalk.green(limitsResults.daysToExhaustQuota.toFixed(1) + ' days')}\n`);

  console.log(chalk.bold('💡 Recommendations:\n'));
  console.log(`  1. Implement per-IP rate limiting: ${chalk.yellow('10 requests per minute')}`);
  console.log(`  2. Implement per-session rate limiting: ${chalk.yellow('100 requests per minute')}`);
