...
async function testConcurrentLoad(concurrentUsers = 10) {
  ...
  const avgLoadTime =
    results.filter((r) => r.success).reduce((sum, r) => sum + r.loadTime, 0) / successful;
...
async function main() {
  ...
  if (successfulResults.length > 0) {
    ...
    const avgTTFB = Math.round(
      successfulResults.reduce((sum, r) => sum + r.ttfb, 0) / successfulResults.length
    );
    const avgFCP = Math.round(
      successfulResults.reduce((sum, r) => sum + r.fcp, 0) / successfulResults.length
    );
    const avgLCP = Math.round(
      successfulResults.reduce((sum, r) => sum + r.lcp, 0) / successfulResults.length
    );
    const avgLoad = Math.round(
      successfulResults.reduce((sum, r) => sum + r.totalTime, 0) / successfulResults.length
    );
    const avgSize = Math.round(
      successfulResults.reduce((sum, r) => sum + r.transferSize, 0) / successfulResults.length
    );
  ...
  if (analyticsRequests.length > 0) {
    console.log(
      chalk.green(`   ✓ Analytics firing correctly (${analyticsRequests.length} requests detected)`)
    );
  } else {
  ...
  console.log(
    `   TTFB: ${avgTTFB}ms ${ttfbPass ? chalk.green("✓") : chalk.red("✗")} (threshold: ${THRESHOLDS.TTFB}ms)`
  );
  console.log(
    `   FCP:  ${avgFCP}ms ${fcpPass ? chalk.green("✓") : chalk.red("✗")} (threshold: ${THRESHOLDS.FCP}ms)`
  );
  console.log(
    `   LCP:  ${avgLCP}ms ${lcpPass ? chalk.green("✓") : chalk.red("✗")} (threshold: ${THRESHOLDS.LCP}ms)`
  );
  console.log(
    `   Load: ${avgLoad}ms ${loadPass ? chalk.green("✓") : chalk.red("✗")} (threshold: ${THRESHOLDS.LOAD_TIME}ms)`
  );
