...
import { testNonNotifiableEvents, testNotifiableEvents, compareResults } from './performance-testing/worker-latency.js';
import { simulateBurstAttack, calculateQuotaExhaustionTime } from './security-testing/rate-limit-test.js';
...
async function runPhase3Security() {
  ...
  try {
    ...
    results.push({ test: 'burst-attack', status: 'completed', accepted: burst.accepted, rejected: burst.rejected });
    ...
    results.push({ test: 'quota-exhaustion', status: 'completed', timeToExhaust: quota.timeToExhaustQuota });
...
async function runPhase4Performance() {
  ...
  try {
    ...
    const dummyContacts = {
      complete: () => {},
      getSummary: () => ({
        latency: { mean: notifiable.getSummary().latency.mean * 1.1, p95: notifiable.getSummary().latency.p95 * 1.1, p99: notifiable.getSummary().latency.p99 * 1.1 }
      })
    };

    const comparison = compareResults(nonNotifiable, notifiable, dummyContacts);
    results.push({ test: 'latency-profiling', status: 'completed', emailOverhead: comparison.emailOverhead });
