  ...
  async scheduled(event, env, ctx) {
    ...
    try {
      ...
      for (const [partnerId, email] of Object.entries(PARTNER_EMAILS)) {
        await generateAndSendReport(env.DB, partnerId, email, startDate, endDate);
      }
...
async function getPartnerStats(db, partnerId, startDate, endDate) {
  ...
  const cardClicks = await db.prepare(`
    ...
    WHERE event_type = 'partner_click'
      ...
      AND timestamp >= ?
      AND timestamp <= ?
  `).bind(partnerId, startISO, endISO).first();
  ...
  const directViews = await db.prepare(`
    SELECT COUNT(*) as count
    FROM analytics_events
  ...
  // Website clicks
  const websiteClicks = await db.prepare(`
    SELECT COUNT(*) as count
    FROM analytics_events
  ...
  const contactSubmissions = await db.prepare(`
    SELECT COUNT(*) as count
    FROM analytics_events
  ...
  const qrScans = await db.prepare(`
  ...
  `).bind(partnerId, startISO, endISO).all();
...
function formatReportEmail(partnerName, stats, startDate, endDate) {
  ...
  if (stats.topQRCodes.length > 0) {
    qrBreakdown = stats.topQRCodes
      .map(qr => `  • ${qr.referrer_id}: ${qr.scans} scan${qr.scans > 1 ? 's' : ''}`)
      .join('\n');
  }
